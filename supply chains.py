...
class Block:
	def __init__ (self, index, previous_hash, current):
		self.index=index
...
class Blockchain:
	...
	def genesis_block(self):
		genesis_block = Block('Genesis', 0x0, [3,4,5,6,7], 'datatime.now().timestamp()', 0)
	...
	def proof_of_work(self, block):
		...
		while not (computed_hash.endswith('0' * difficulty) and ('55' * difficulty) in computer_hash):
			block.nonce += 1
			computed_hash = block.compute_hash()
			return compute_hash
				...
				try:
					if id == 'all':
						for i in range(len(self. transactions)-1):
							print('{}:\n{}\n'.format(lables[i], self.transactions[i+1]))
							break
